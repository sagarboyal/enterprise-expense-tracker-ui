...
const EnterpriseIntro = ({ theme }) => {
  ...
  return (
    ...
    >
      {/* HERO SECTION */}
      <section className="relative min-h-[calc(100vh-4.5rem)] flex items-center justify-center overflow-hidden selection:bg-indigo-500/30 px-4 py-20 sm:px-6 lg:px-8">
        <div className="absolute left-1/2 top-0 z-0 h-full w-full -translate-x-1/2 pointer-events-none">
          <motion.div
            animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }}
            transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
            className={`absolute top-[-10%] left-[-10%] h-[40%] w-[40%] rounded-full blur-[120px] ${isDark ? "bg-indigo-600/20" : "bg-indigo-300/40"}`}
          />
          <motion.div
            ...
            className={`absolute bottom-[-10%] right-[-10%] h-[40%] w-[40%] rounded-full blur-[120px] ${isDark ? "bg-purple-600/10" : "bg-fuchsia-200/50"}`}
          />
        </div>
        ...
        >
          <motion.div variants={itemVariants}>
            <span
              className={`inline-block rounded-full border px-5 py-2 text-xs font-bold uppercase tracking-widest backdrop-blur-md mb-8 ${isDark ? "border-indigo-500/30 bg-indigo-500/10 text-indigo-300" : "border-indigo-200 bg-white/60 text-indigo-700 shadow-sm"}`}
            >
              🏢 Enterprise Edition
            </span>
          </motion.div>
          ...
          >
            Stop chasing receipts and sprawling spreadsheets. Start scaling your
            business with an intelligent, all-in-one financial hub designed for
          ...
          >
            ...
            >
              <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-70 blur-md transition-all duration-500 group-hover:opacity-100 group-hover:duration-200 group-active:scale-95"></div>
              <span
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  <div
                    className={`absolute -inset-4 rounded-3xl opacity-30 blur-2xl transition-all duration-500 bg-gradient-to-br from-indigo-500 to-purple-500 ${feature.glow}`}
                  ></div>
                  <img
                    src={feature.image}
                    alt={feature.title}
      ...
      <section className="relative z-10 px-4 py-24 sm:px-6 lg:px-8 border-t border-slate-200/50 dark:border-white/5 mt-16">
        ...
        >
          ...
          >
            Join thousands of modern organizations using TREX to manage
            expenses, approve reimbursements, and analyze financial data with
            unmatched clarity.
          </p>
          <Link
            to="/register"
            className="inline-flex items-center justify-center rounded-full bg-indigo-600 px-8 py-4 text-base font-bold text-white shadow-lg shadow-indigo-500/30 transition-all hover:bg-indigo-500 hover:scale-105 active:scale-95"
          >
