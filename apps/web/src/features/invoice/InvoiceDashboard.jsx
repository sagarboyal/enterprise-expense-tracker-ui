...
import { FaPlus, FaSearch, FaUser, FaEnvelope } from "react-icons/fa";
import api from "@/services/api";
import ViewInvoiceModal from "@/features/invoice/ViewInvoiceModal";
...
const InvoiceCardSkeleton = () => (
  <Card
    sx={{ borderRadius: 4, boxShadow: "none", border: "1px solid #e0e0e0" }}
  >
    <CardContent sx={{ p: 3 }}>
    ...
    <div className="min-w-7xl flex items-center justify-center pl-4">
      <div className="w-full max-w bg-white shadow-sm rounded-xl border border-gray-200/75 p-8 text-center">
        ...
        <p className="text-gray-500 max-w-xs mx-auto mb-6">
          It seems there are no invoices to display. Try adjusting your filters
...
const InvoiceDashboard = () => {
  ...
  const fetchInvoices = React.useCallback(
    async (pageNumber, currentFilters) => {
      setLoading(true);
      try {
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 3, color: "text.secondary" }}>
            Enter the user's email to generate a new invoice. The invoice will
            be created with an "In Progress" status.
          </Typography>
          <TextField
            autoFocus
            margin="dense"
